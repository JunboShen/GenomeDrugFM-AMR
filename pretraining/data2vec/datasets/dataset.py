...
class DatasetPretrain(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        embs_tensor = torch.tensor(genome_embs, dtype=torch.float32)  # (L, D)
        embs_seg_ids = torch.tensor(genome_seg_ids, dtype=torch.long) # (L,)
...
class DatasetForTasks(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        genome_embs = np.load(os.path.join(self.root_path, str(self.genome_ids[idx]) + '.npy'))
        if self.kmer_root_path:
            kmer_dict = np.load(
                os.path.join(self.kmer_root_path, str(self.genome_ids[idx]) + ".npy"),
                allow_pickle=True,
            ).item()
            genome_kmer = list(kmer_dict.values())
            genome_kmer_tensor = torch.tensor(genome_kmer, dtype=torch.float32)
        else:
            genome_kmer_tensor = torch.zeros(512, dtype=torch.float32)
        ...
        return {'embs': embs_tensor, 'labels': label_tensor, 'genome_kmer': genome_kmer_tensor}
