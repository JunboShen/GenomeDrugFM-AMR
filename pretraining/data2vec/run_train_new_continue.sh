...
# Continue GenomeData2Vec pretraining from an existing checkpoint.
#
# Required:
...
: "${RESUME_CHECKPOINT:?Set RESUME_CHECKPOINT=/path/to/checkpoint.pth}"
...
: "${WARMUP_EPOCHS:=0}"
: "${MIN_LR:=5e-7}"
: "${OUTPUT_DIR:=./outputs/pretrain_patch4096_continue}"
: "${SAVECKP_FREQ:=10}"
: "${SEED:=0}"
: "${NUM_WORKERS:=8}"
: "${GC:=32}"
...
"${PYTHON}" -u "${SCRIPT_DIR}/train_new_continue.py" \
