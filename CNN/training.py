...
from model import MDCNN_MIL_Drug
...
def train(dataloader, fold, args):
    ...
    # set up the model
    #model = CNNTransMIL(input_dim=1536+768, n_classes= args.n_classes, input_dim_kmer=512,  patch_size=4096, in_chans=4, embed_dim=1536)
    model = MDCNN_MIL_Drug(seg_len=4096, drug_dim=768, n_classes=args.n_classes)
...
def train_one_epoch(train_loader, model, fp16_scaler, optimizer, loss_fn, epoch, args):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        genome_embs_tensor, embs, label, genome_kmer = batch['embs'], batch['drug_emb'],batch['labels'],batch['genome_kmer']
        #embs, label, genome_kmer = batch['embs'], batch['labels'], batch['genome_kmer']
        genome_embs_tensor = genome_embs_tensor.to(args.device, non_blocking=True)
        genome_kmer = genome_kmer.to(args.device, non_blocking=True)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16 if args.fp16 else torch.float32):
            # get the logits
            logits, _= model(genome_embs_tensor, embs)
            # get the loss
