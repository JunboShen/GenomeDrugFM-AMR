...
def train(args, cfg, wandb_run=None):
    ...
    eff_batch_size = args.batch_size_per_gpu * args.gc
    # ============ init schedulers ... ============
    ...
    to_restore = {"epoch": 0}
    resume_checkpoint = _resolve_path(args.resume_checkpoint) or ""
    if not resume_checkpoint:
        raise ValueError("--resume_checkpoint is required for train_new_continue.py")
    ...
    model.ema.model = model.ema.deepcopy_model(model.encoder)
...
def train_one_epoch(model, data_loader,
    ...
    for it, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        # student update
        # Note: with gradient accumulation we only step/zero every `accumulate` iterations.
        param_norms = None
        if fp16_scaler is None:
            loss_to_backprop.backward()
            if ((it + 1) % accumulate == 0) or (it + 1 == len(data_loader)):
                if args.clip_grad:
            ...
            fp16_scaler.scale(loss_to_backprop).backward()
            if ((it + 1) % accumulate == 0) or (it + 1 == len(data_loader)):
                if args.clip_grad:
