...
# def pad_2d_tensors(tensors: list[torch.Tensor]) -> torch.Tensor:
#     """Pad a list of (Li, D) float tensors to (B, Lmax, D) with zeros."""
#     B = len(tensors)
#     D = tensors[0].size(1)
#     Lmax = max(t.size(0) for t in tensors)
#     out = torch.zeros(B, Lmax, D, dtype=tensors[0].dtype)
#     for i, t in enumerate(tensors):
#         out[i, :t.size(0)] = t
#     return out

def pad_3d_tensors(emb_list):
    """
    emb_list: list of tensors with shape [Li, 4096, 4]
    return:   tensor [B, Lmax, 4096, 4] (zero-padded on Li)
    """
    B = len(emb_list)
    S = emb_list[0].size(1)  # 4096
    C = emb_list[0].size(2)  # 4
    lengths = torch.tensor([e.size(0) for e in emb_list], dtype=torch.long)    # [B]
    Lmax = int(lengths.max().item())

    out = torch.zeros(B, Lmax, S, C, dtype=emb_list[0].dtype)                  # CPU in collate
    for i, e in enumerate(emb_list):
        Li = e.size(0)
        out[i, :Li] = e
    # [B, Lmax] bool, where pad is True
    pad_mask = torch.arange(Lmax).unsqueeze(0) >= lengths.unsqueeze(1)
    return out, lengths, pad_mask

def pad_1d_tensors(tensors: list[torch.Tensor], pad_value: int=0) -> torch.Tensor:
    """Pad a list of (Li,) long tensors to (B, Lmax) with pad_value."""
    B = len(tensors)
    Lmax = max(t.size(0) for t in tensors)
    out = torch.full((B, Lmax), pad_value, dtype=tensors[0].dtype)
    for i, t in enumerate(tensors):
        out[i, :t.size(0)] = t
    return out
...
def collate_pretrain(samples):
    # lists of shapes
    embs_list = [s['embs'] for s in samples]                  # each (Li, D, 4), long
    segid_list = [s['embs_seg_ids'] for s in samples]         # each (Li,), long

    # pad embs to (B, Lmax, D, 4
    embs, lengths, pad_mask = pad_3d_tensors(embs_list)  # embs: float32, (B, Lmax, D, 4)

    # pad seg ids to (B, Lmax)
    seg_ids = pad_1d_tensors(segid_list)

    return {
        'embs': embs,                       # float32, (B, Lmax, D)
        'embs_seg_ids': seg_ids,            # long,    (B, Lmax), PAD_SEG_ID where padded
        'lengths': lengths,                 # long,    (B,)
        'pad_mask': pad_mask,               # bool,    (B, Lmax), True where padded
    }
